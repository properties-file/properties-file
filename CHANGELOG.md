...
- **ES5 runtime compatibility restored for the full API.** The compiled output of every previous 5.x release contained unguarded `Symbol.iterator` lookups, so on ES5-only engines (anything without a `Symbol` global — e.g. Node.js < 0.12): `PropertiesEditor`, `Properties#toObject`, `Properties#format`, and duplicate-key handling crashed with `Symbol is not defined` — only `getProperties` and the escape/unescape utilities worked there. The published artifacts no longer contain any unguarded `Symbol` reference, and the full API works on every supported runtime, back to Node.js 0.4.0.

### Features

- Every release is now **behaviorally verified as built**: a golden test suite of 72 scenarios (parsing, formatting, editing, escaping — including astral characters) executes the compiled `dist/` artifacts, not just the source, on runtimes from Node.js 0.4.0 to current. This verification is what caught the bug above.
- Smaller bundles: `Properties` −7.5% min+gzip (3.1 kB → 2.9 kB), `PropertiesEditor` −4.3% (5.1 kB → 4.8 kB). `getProperties` stays at 970 B.
